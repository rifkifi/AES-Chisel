...
class AESIO(key_size: Int) extends Bundle {
  val in_data = Input(UInt(128.W))
  val in_key = Input(UInt(key_size.W))
  val start = Input(Bool())
  val done = Output(Bool())
  val out = Output(UInt(128.W))
}

class AES(key_size: Int) extends Module {
  val io = IO(new AESIO(key_size))

  val (maxWords, maxRounds, nK) = key_size match {
    case 256 => (60, 14, 8)
    case 192 => (52, 12, 6)
    case 128 => (44, 10, 4)
    case _   => throw new Exception(s"Unsupported key size: $key_size")
  }
  ...
  // Registers
  val W = Wire(Vec(maxWords, UInt(32.W)))
  val round = RegInit(0.U(4.W))
  val roundKey = RegInit(VecInit(Seq.fill(maxRounds + 1)(0.U(128.W))))
  val dataState = Reg(Vec(4, Vec(4, UInt(8.W)))) // state(row)(col)
  W := VecInit(Seq.fill(maxWords)(0.U(32.W)))
  ...
  switch(state) {
    is(sIdle) {
      when(io.start) {
        ...
        for (i <- 0 until nK) { 
          W(i) := io.in_key((key_size - 1) - i * 32, (key_size - 32) - i * 32) 
        }
        for (i <- nK until maxWords) {
          if (i % nK == 0) {
            W(i) := W(i - nK) ^ subWord(rotWord(W(i - 1))) ^ rconWord((i / nK).U)
          } else if ((nK > 6) && (i % nK == 4)) {
    ...
    is(sRoundCore) {
      // round core process consist of subBytes, shiftRows, mixColumns and addRoundKey
      val sb = subBytes(dataState)
      val sr = shiftRows(sb)
      val mix = mixColumns(sr)
      dataState := addRoundKey(mix, roundKey(round))
      when (round === (maxRounds - 1).U) {
        round := maxRounds.U
        state := sFinalRound
      }.otherwise {
        round := round + 1.U   
